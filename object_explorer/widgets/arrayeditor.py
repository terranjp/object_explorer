...
from utilities.utils import (is_binary_string, is_text_string, to_binary_string, is_string)
...
class ArrayEditorWidget(QWidget):
    def __init__(self, parent, data, readonly=False, xlabels=None, ylabels=None):
        ...
        self.model = ArrayModel(self.data, format=format, xlabels=xlabels, ylabels=ylabels, readonly=readonly, parent=self)
    ...
    def change_format(self):
        """Change display format"""
        format, valid = QInputDialog.getText(self,  'Format', "Float formatting", QLineEdit.Normal, self.model.get_format())
        if valid:
            format = str(format)
            try:
                format % 1.1
