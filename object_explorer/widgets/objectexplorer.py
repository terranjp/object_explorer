...
# from utilities.utils import _

from utilities import utils
from utilities import qthelpers

# from utilities.qthelpers import create_action, create_toolbutton

from .collectionseditor import CollectionsEditorTableView

class ObjectExplorer(QWidget):
    """Namespace browser (global variables explorer widget)"""
    sig_option_changed = pyqtSignal(str, object)
    sig_collapse = pyqtSignal()
    ...
    def setup(self,
        ...
        self.editor = CollectionsEditorTableView(self, data=data, minmax=minmax)
        ...
        # Options menu
        options_button = qthelpers.create_toolbutton(self, text=utils._('Options'))

        options_button.setPopupMode(QToolButton.InstantPopup)
        menu = QMenu(self)
        editor = self.editor
        actions = [self.exclude_private_action,
    ...
    def setup_toolbar(self,
        ...
        self.exclude_private_action = qthelpers.create_action(self,
                                                    utils._("Exclude private references"),
                                                    tip=utils._("Exclude references which name starts"
                                                          " with an underscore"),
                                                    toggled=lambda state:
                                                    self.sig_option_changed.emit('exclude_private', state))
        self.exclude_private_action.setChecked(exclude_private)

        self.exclude_uppercase_action = qthelpers.create_action(self,
                                                                utils._("Exclude all-uppercase references"),
                                                      tip=utils._("Exclude references which name is uppercase"),
                                                      toggled=lambda state:
                                                      self.sig_option_changed.emit('exclude_uppercase', state))
        self.exclude_uppercase_action.setChecked(exclude_uppercase)

        self.exclude_capitalized_action = qthelpers.create_action(self,
                                                                  utils._("Exclude capitalized references"),
                                                        tip=utils._("Exclude references which name starts with an "
                                                              "uppercase character"),
                                                        toggled=lambda state:
                                                        self.sig_option_changed.emit('exclude_capitalized', state))
        self.exclude_capitalized_action.setChecked(exclude_capitalized)

        self.exclude_unsupported_action = qthelpers.create_action(self,
                                                        utils._("Exclude unsupported data types"),
                                                        tip=utils._("Exclude references to unsupported data types"
