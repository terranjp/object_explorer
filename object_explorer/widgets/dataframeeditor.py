...
from . arrayeditor import get_idx_rect

from utilities.utils import (io, is_binary_string, is_string, is_text_string, _, TEXT_TYPES
                             )
...
class DataFrameModel(QAbstractTableModel):
    ...
    def headerData(self, section, orientation, role=Qt.DisplayRole):
        ...
        if orientation == Qt.Horizontal:
            ...
            # else:
                ...
                #     coding = encoding.get_coding(header)
                return to_qvariant(str(header, encoding=coding))
            else:
                return to_qvariant(str(self.df_header[section-1]))
        else:
            return to_qvariant()
    ...
    def data(self, index, role=Qt.DisplayRole):
        ...
        if role == Qt.DisplayRole or role == Qt.EditRole:
            ...
            row = index.row()
            if column == 0:
                return str(self.df_index[row])
            else:
                value = self.get_value(row, column-1)
                if isinstance(value, float):
                    return f'{value:.{self.precision}{self.format_type}}'
                else:
                    try:
                        return str(value)
...
class DataFrameView(QTableView):
    ...
    def moveCursor(self, cursor_action, modifiers):
        ...
        col_width = (self.frozen_table_view.columnWidth(0) + self.frozen_table_view.columnWidth(1))
