...
from utilities.qthelpers import create_action

# from widgets.importwizard import ImportWizard
from . texteditor import TextEditor
from . quantityeditor import QuantityArrayEditor
...
class ReadOnlyCollectionsModel(QAbstractTableModel):
    ...
    def set_data(self, data, coll_filter=None):
        ...
        if coll_filter is not None and not self.remote and isinstance(data, (tuple, list, dict)):
        ...
        if isinstance(data, tuple):
            ...
            if not self.names:
                self.header0 = "Key"
        elif isinstance(data, _Quantity):
    ...
    def headerData(self, section, orientation, role=Qt.DisplayRole):
        ...
        if orientation == Qt.Horizontal:
            ...
            headers = self.header0, "Type", "Size", "Value"
...
class CollectionsDelegate(QItemDelegate):
    ...
    def createEditor(self, parent, option, index):
        ...
        try:
            ...
            QMessageBox.critical(self.parent(), "Error",
                                 "Spyder was unable to retrieve the value of this variable from the console.<br><br>"
                                 "The error mesage was:<br> <i>%s</i>") % str(msg)
            return

        key = index.model().get_key(index)
        readonly = isinstance(value, tuple) or self.parent().readonly or not is_known_type(value)
        ...
        # ---showing image
        elif isinstance(value, Image) and ndarray is not FakeObject and Image is not FakeObject:
            arr = array(value)
            editor = ArrayEditor(parent)
        ...
        elif isinstance(value, (DataFrame, DatetimeIndex, Series)) and DataFrame is not FakeObject:
            editor = DataFrameEditor()
    ...
    def setEditorData(self, editor, index):
        ...
        elif isinstance(editor, QuantityEditor):
    ...
    def setModelData(self, editor, model, index):
        ...
        if isinstance(editor, QLineEdit):
            ...
            try:
                ...
                raise
                QMessageBox.critical(editor, _("Edit item"), ("<b>Unable to assign data to item.</b>"
            ...
            value = datetime.datetime(qdate.year(), qdate.month(), qdate.day(), qtime.hour(), qtime.minute(), qtime.second())
        else:
            # Should not happen...
...
class BaseTableView(QTableView):
    ...
    def copy_item(self, erase_original=False):
        ...
        if erase_original:
            title = 'Rename'
            field_text = 'New variable name:'
        else:
            title = 'Duplicate'
            field_text = 'Variable name:'
    ...
    def insert_item(self):
        ...
        if isinstance(data, list):
            ...
            key, valid = QInputDialog.getText(self, 'Insert', 'Key:', QLineEdit.Normal)
            ...
            return
        value, valid = QInputDialog.getText(self, 'Insert', 'Value:', QLineEdit.Normal)
    ...
    def __prepare_plot(self):
        try:
            ...
            try:
                ...
                QMessageBox.warning(self, "Import error", "Please install <b>matplotlib</b> or <b>guiqwt</b>.")
    ...
    def plot_item(self, funcname):
        ...
        if self.__prepare_plot():
            ...
            try:
                ...
                QMessageBox.critical(self, "Plot", "<b>Unable to plot data.</b> <br><br>Error message:<br>%s") % str(error)
    ...
    def imshow_item(self):
        ...
        if self.__prepare_plot():
            ...
            try:
                ...
                QMessageBox.critical(self, "Plot",
                                     "<b>Unable to show image.</b>"
                                       "<br><br>Error message:<br>%s"
                                       ) % str(error)
    ...
    def paste(self):
        ...
        if cliptext.strip():
            self.import_from_string(cliptext, title="Import from clipboard")
        else:
            QMessageBox.warning(self, "Empty clipboard","Nothing to be imported from clipboard.")
...
class CollectionsEditorTableView(BaseTableView):
    ...
    def __init__(self,
        ...
        self.model = CollectionsModel(self, data, title, names=names, minmax=minmax)
    ...
    def refresh_menu(self):
        ...
        data = self.model.get_data()
        index = self.currentIndex()
        condition = (not isinstance(data, tuple)) and index.isValid() and not self.readonly
        self.edit_action.setEnabled(condition)
        self.remove_action.setEnabled(condition)
...
class CollectionsEditor(QDialog):
    ...
    def setup(self, data, title='', readonly=False, width=650, remote=False,
        ...
        self.widget = CollectionsEditorWidget(self, self.data_copy, title=title, readonly=readonly, remote=remote)
