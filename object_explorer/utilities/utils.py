...
def get_size(item):
    ...
    if isinstance(item, (list, tuple, dict)):
        ...
        return item.shape
    elif isinstance(item, _Quantity):
...
def value_to_display(value, minmax=False):
    ...
    try:
        ...
        if isinstance(value, recarray):
            ...
            display = value
        elif isinstance(value, NUMERIC_TYPES) or isinstance(value, bool) or isinstance(value, datetime.date) or \
...
def display_to_value(value, default_value, ignore_errors=True):
    ...
    try:
        ...
        elif isinstance(default_value, _Quantity):
            try:
                value = int(value)
            except ValueError:
                value = float(value)
...
def get_supported_types():
    ...
    editable_types = [int, float, complex, list, dict, tuple, date] + list(TEXT_TYPES) + list(INT_TYPES)
    try:
